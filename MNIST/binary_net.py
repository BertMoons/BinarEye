...
from augment import adapt_to_binareye
...
def train(train_fn,val_fn,
    ...
    def train_epoch(X,y,LR):
        ...
        for i in range(batches):
            train_x_ = X[i*batch_size:(i+1)*batch_size].transpose([0,2,3,1])
	    train_x_ = augment_images(train_x_,rotation_range=30,height_shift_range=0.1, width_shift_range=0.1,shear_range=0.1,zoom_range=(1,1))
	    train_x_ = adapt_to_binareye(train_x_.transpose([0,3,1,2]),filters=64)
            ...
            loss += train_fn(train_x_,y[i*batch_size:(i+1)*batch_size],LR)
