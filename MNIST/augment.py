...
def adapt_to_binareye(x, filters=64):
	y = np.subtract(np.multiply(2.,x),1.)
	# Number of feature maps
	num_maps = np.floor(256/(1*256/filters)) # 256/3 input channels
	# Quantize
	s = y / np.abs(y)
	y=(2*(s*np.ceil(np.abs(y)*num_maps/2))-s*1).astype('float32')
	return y
