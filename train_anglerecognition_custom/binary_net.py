...
def train(train_fn,val_fn,
    ...
    def val_epoch(X,y):
        ...
        y_pred = np.asarray([])
        for i in range(batches):
            new_loss, new_err, new_y_pred = val_fn(X[i*batch_size:(i+1)*batch_size], y[i*batch_size:(i+1)*batch_size])
            err += new_err
            loss += new_loss
    ...
    for epoch in range(num_epochs):
        ...
        val_err, val_loss, _ = val_epoch(X_val,y_val)
        ...
        if val_err <= best_val_err:
            ...
            test_err, test_loss, test_pred = val_epoch(X_test,y_test)
            
            if save_path is not None:
                np.savez(save_path, *lasagne.layers.get_all_param_values(model))
            # Then we print the results for this epoch:
            #y_actual = np.argmax(y_test,axis=1)
            #test_pred = np.reshape(test_pred,(test_pred.shape[0]/3,3))
        ...
        #print("  test mAP:                      "+str(mapk(y_actual, test_pred, test_pred.shape[0]))) 
